...
> **v1.3**：新增 §6.4 片段提取三级产物（区间播放 / 预览草稿 / 原素材精切）。
...
### 6.4 片段提取三级产物（v1.3 新增）

从长视频提取某个时间段，按"画质 / 速度 / 是否动原片"分三级。核心原则：**绝大多数标注和浏览场景不需要导出文件**，因为 labels.jsonl 已存了精确起止点，原片随时可重切。

| 产物 | 来源 | 提取耗时 | 画质 | 体积 | 用途 |
|---|---|---|---|---|---|
| **区间播放** | 本地 proxy | 0 秒（不生成文件） | 720p | 0 | 浏览、微调起止点 |
| **预览草稿** | 本地 proxy | 秒级（copy 切） | 720p | 几 MB | 分享、快速确认 |
| **原素材精切** | NAS 原片 | 数十秒（重编码） | 原画质 | 几十 MB | 集锦、存档、上传 |

#### 6.4.1 关键技术点：copy vs re-encode

| 模式 | 命令核心 | 速度 | 精度 | 画质 |
|---|---|---|---|---|
| 流拷贝 | `-c copy` | 极快（不解码） | 只能切到关键帧（GOP 边界），偏差 1-5s | 无损 |
| 重编码 | `-c:v libx264` | 慢（解码+编码） | 精确到帧 | 有损可控 |

预览用 copy（够用），最终成片用 re-encode（精确）。

#### 6.4.2 区间播放（零提取，最快）

浏览器 HTML5 video 直接对 proxy 做区间播放，不切文件：

```javascript
video.src = "/proxy/<sha6>.mp4";
video.currentTime = start_ts;
video.play();
video.ontimeupdate = () => { if (video.currentTime >= end_ts) video.pause(); };
```

拖时间轴 = 改 currentTime。这是微调起止点的主力手段。

#### 6.4.3 预览草稿（秒级 copy 切）

要单独存成文件分享时，从 proxy 秒切：

```bash
ffmpeg -ss <start> -i proxy/<sha6>.mp4 -t <dur> \
  -c copy -avoid_negative_ts make_zero \
  preview/<rally_id>-draft.mp4
```

`-ss` 置于 `-i` 前 = 快速 seek。720p、几 MB，适合微信分享。

#### 6.4.4 原素材精切（重编码，原画质）

仅在做正式集锦/存档/上传时触发，从 NAS 原片精切：

```bash
ffmpeg -ss <start> -i nas/<原片> -t <dur> \
  -c:v libx264 -crf 18 -preset slow \
  -c:a aac -b:a 192k \
  exports/<rally_id>.mp4
```

多段合集走 concat：逐段精切到临时文件 → 生成 concat 清单 → `ffmpeg -f concat -c copy` 合并。

#### 6.4.5 何时动原片（决策表）

| 场景 | 源 | 模式 |
|---|---|---|
| 日常浏览/标注 | proxy | 区间播放（不切） |
| 发球友看一眼 | proxy | copy 秒切 |
| 自己复盘存档 | proxy | copy 秒切 |
| 正式集锦/上传 | NAS 原片 | 重编码精切 |
| 高清截图帧 | NAS 原片 | 抽帧 |

#### 6.4.6 默认编码参数（按出口分档）

| 出口 | crf | preset | 容器/编码 | 说明 |
|---|---|---|---|---|
| 个人存档 | 18-20 | slow | mp4 / H.264 | 高画质，体积较大 |
| 社交平台分享 | 22-23 | medium | mp4 / H.264 + AAC | 兼容性优先，体积适中 |
| 微信/群发 | 24 | fast | mp4 / H.264 | 小体积优先 |

> 出口默认值待用户确认主要用途后冻结（见第 11 节 Q7）。
...
7. **导出主要用途（v1.3 新增）**：集锦主要是个人存档、社交平台分享、还是微信群发？决定 §6.4.6 默认编码参数的冻结值。
...
### v1.3 · 2026-06-18
- 新增 §6.4 片段提取三级产物（区间播放 / 预览草稿 / 原素材精切）。
- 明确 copy vs re-encode 的取舍：预览用 copy（基于本地 proxy，秒级），成片用 re-encode（从 NAS 原片精切）。
- 核心原则：labels.jsonl 已存精确起止点，绝大多数场景不需导出文件。
- 给出区间播放 / copy 切 / 精切的 ffmpeg 命令与决策表。
- 新增 §6.4.6 按出口分档的默认编码参数（待 Q7 确认冻结）。
